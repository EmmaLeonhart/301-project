...
<section id="questions">
  <div class="container">
    ...
    <ol>
      <li><strong>Do Wikipedia categories line up with the Wikidata ontology?</strong> &mdash; We follow Wikidata's P910 (topic's main category) link from each P31 (instance of) class to its corresponding Wikipedia category and check whether items are actually placed in that category.</li>
      <li><strong>Do they align more in certain domains?</strong> &mdash; We sample five domains (animals, films, cities, chemical elements, albums) and compare how many steps up the Wikipedia category tree are needed to find the P910 (topic's main category) predicted category.</li>
...
<section id="methodology">
  <div class="container">
    ...
    <div class="card-grid">
      <div class="card">
        <h4>Wikidata</h4>
        <p>SPARQL queries against the <a href="https://query.wikidata.org/">Wikidata Query Service</a> fetch items that are <code>P31 (instance of)</code> a target class, their P31 (instance of) values, and the <code>P910 (topic's main category)</code> links on those classes.</p>
      </div>
      <div class="card">
...
<section id="pipeline">
  <div class="container">
    ...
    <p><code>src/wikidata.py</code> sends SPARQL queries to fetch P31 (instance of) items with English Wikipedia sitelinks, then resolves <code>P910 (topic's main category)</code> links for each P31 (instance of) class (walking up <code>P279 (subclass of)</code> if needed). <code>src/wikipedia.py</code> fetches categories and parent-category chains via the Wikipedia API.</p>

    <h3>Step 2: Merge &amp; Clean</h3>
    <p><code>src/etl.py</code> joins Wikidata P31 (instance of) / P910 (topic's main category) data with Wikipedia categories into a single DataFrame, computing depth to P910 (topic's main category) match for each item.</p>
...
{domain_table_rows}      </tbody>
    ...
    <ol>
      <li><strong>Chemical elements align directly.</strong> The P910 (topic's main category) predicted category appears on every item at depth 0. This domain is small, well-defined, and tightly curated on both systems.</li>
      <li><strong>Albums and cities need 2&ndash;3 steps.</strong> Wikipedia files albums under year/genre subcategories ("2024 pop albums") and cities under geographic subcategories ("Cities in Bavaria") rather than the broad P910 (topic's main category).</li>
    ...
    <div class="card-grid">
      <div class="card">
        ...
        <p><strong>Sometimes directly, always eventually.</strong> Following P910 (topic's main category) links, every domain converges &mdash; the question is how many steps up the Wikipedia category tree it takes. Chemical elements match at depth 0; animals need ~5 levels.</p>
      </div>
      <div class="card">
...
<section id="ethics">
  <div class="container">
    ...
    <ul>
      ...
      <li><strong>P910 (topic's main category) coverage:</strong> The analysis relies on P31 (instance of) classes having P910 (topic's main category) properties. Classes without P910 (topic's main category) are invisible to this comparison.</li>
